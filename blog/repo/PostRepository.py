...
class PostRepository:
    ...
    def get_all(self):        
        ...
        try:
            objResult: QuerySet[Posts] = Posts.objects.filter(is_active=True).values('id','post_title','post_content','created_at','updated_at','author','featured_image','post_summary','slug','is_featured_post');
            all_posts = [
                {
                    'id' : int(item['id']),
    ...
    def get_featured_post(self) -> Union [Posts | None]:
        ...
        try:
            objResult: Posts = Posts.objects.filter(is_active=True, is_featured_post=True).values('id','post_title','post_content','created_at','updated_at','author','featured_image','post_summary','slug','is_featured_post').last()
            featured_post = {
                    'id' : int(objResult['id']),
